"""
Scoring Engine v12.0 — Unified scoring with deep factors & ATR regime
======================================================================
v12.0 增强 (移植自 loop-win-2026):
  - 深度因子融合: 残差动量、IVOL、封板强度、尾盘异动 (20%权重)
  - ATR市场状态感知: CALM/VOLATILE/PANIC 三档自适应权重
  - VWAP评分: 价格在VWAP上方加分
  - 拥挤度过滤: 高相关性股票降分
  - 反转策略支持: 新增strategy_tag区分
...
# v12.0: 深度因子权重 (从总分100中额外分配)
DEEP_FACTOR_WEIGHT = 20  # 深度因子占20分，原有因子按比例压缩至80分
...
# ─── v12.0: Enhanced composite score with deep factors ───

def calc_enhanced_composite(ag_score, micro_mom_score, inst_score, vcp_score,
                            deep_score=None, vwap_score=None,
                            regime='neutral'):
    """
    增强版综合评分 (0-100):
      - 原有因子占80%权重 (按regime调整)
      - 深度因子占15%权重
      - VWAP占5%权重

    Args:
        deep_score: 深度因子综合分 (0-100)，来自DeepFactorEngine
        vwap_score: VWAP位置分 (0-10)，来自VWAPCalc
        regime: 市场状态
    """
    w = get_regime_weights(regime)
    ...
    # 原有因子部分 (压缩到80分)
    base = 0.0
    base += min(inst_score, w['inst_burst'])
    base += min(micro_mom_score, w['micro_mom'])
    base += min(ag_score * 2.0, float(w['antigravity']))
    base += min(vcp_score, w['vcp'])
    base = min(base, 80.0)

    # 深度因子部分 (15分)
    deep_part = 0.0
    if deep_score is not None:
        deep_part = (deep_score / 100.0) * 15.0

    # VWAP部分 (5分)
    vwap_part = 0.0
    if vwap_score is not None:
        vwap_part = (vwap_score / 10.0) * 5.0

    total = base + deep_part + vwap_part
    return round(min(total, 100.0), 1)


def get_enhanced_confidence_grade(composite_score, regime='neutral'):
    """
    增强版信心评级 (考虑市场状态)
    ...
    """
    # 市场状态阈值调整
    regime_offset = {'neutral': 0, 'bull': -5, 'bear': 10}.get(regime, 0)

    adjusted = composite_score - regime_offset
